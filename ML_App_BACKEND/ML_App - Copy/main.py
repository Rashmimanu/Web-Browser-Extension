...
def train_model():
    ...
    if train_needed is None or browser_url == "":
        ...
        blacklisted_status = check_black_listed(browser_url)

        # if the url domain is whitelisted
        if whitelisted_status:
            print("The URL's domain is white listed")
            prediction = "good"

        # if the url domain is blacklisted
        elif blacklisted_status:
            print("The URL's domain is Blacklisted")
            prediction = "bad"

        else:
            print("The URL's domain is NOT white listed or Blacklisted")
...
# This function is used to check whether url domain is Blacklisted
def check_black_listed(browser_url):
    print("===== check_black_listed function is running =====")
    ...
    try:
        ...
        for i in white_list:
            if i == domain:
                print("the domain is Blacklisted")
                blacklisted_status = True
                break
    except:
        blacklisted_status = False

    return blacklisted_status
...
# function add data into blacklist.txt file
@app.route('/blacklist/add', methods=['POST'])
def add_to_black_list():
    print("===== add_to_black_list function is running =====")
    ...
    try:
        if browser_url != "":
            domain = domain_breaker(browser_url)
            with open('blacklist.txt', 'a', newline='') as textFile:
                # creating a csv dict writer object
                row = [domain]
                writer = csv.writer(textFile)
                writer.writerow(row)
                print("Domain Blacklisted successfully")

        else:
            print("Url is null. Blacklisting failed!")
